...
describe('TermsOfUseScreen', () => {
  ...
  it('le lien inline "politique de confidentialité" est accessible et navigue réellement vers la page privacy', async () => {
    ...
    // Même route ("Legal"), nouveaux params -> LegalWebViewScreen re-rend en WebView(privacy).
    // La transition native-stack est asynchrone même en environnement de test -> waitFor.
    await waitFor(() =>
      expect(getByTestId('webview').props.source.uri).toBe('https://www.tonnkama.com/privacy-policy'),
    );
