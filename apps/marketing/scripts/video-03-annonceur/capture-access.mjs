/**
 * Video 03.1 - Acceder a Trouve Ton Nkama.
 *
 * Sortie:
 *   videos/video-03-annonceur/final/video-03-01-acces-plateforme.mp4
 */
...
function cleanup() {
  for (const file of fs.readdirSync(CAPTURES)) {
    if (file.startsWith('video-03-01-access')) {
...
async function capture() {
  ...
  const webm = path.join(CAPTURES, 'video-03-01-access.webm');
  fs.renameSync(tmp, webm);
  return webm;
}

function convert(webm) {
  const out = path.join(FINAL, 'video-03-01-acces-plateforme.mp4');
  execSync(
...
async function main() {
  console.log('Capture video-03-01 acces plateforme');
