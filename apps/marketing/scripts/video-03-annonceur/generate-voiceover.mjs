/**
 * Video 03 annonceur - Voix off synchronisee.
 *
 * Sorties:
 *   videos/video-03-annonceur/voiceover/voiceover-final.mp3
 *   videos/video-03-annonceur/final/video-03-annonceur-complet-voiceover.mp4
 */
...
const SEGMENTS = [
  {
    ...
    rate: 0.92,
    ssml: `<speak>
      Voici comment utiliser Trouve Ton Nkama en tant qu'annonceur.
    </speak>`,
  },
  {
    id: 'access',
  ...
  },
  {
    ...
    ssml: `<speak>
      ...
      <break time="0.7s"/>
      Indiquez ensuite les détails du logement, comme le nombre de chambres, la cuisine, la douche et les toilettes.
      <break time="0.7s"/>
      Enfin, choisissez la province, la ville et le quartier, puis ajoutez les informations utiles pour retrouver le logement.
      <break time="0.6s"/>
...
async function main() {
  console.log('Voix off video-03-annonceur');
  ...
  const videoIn = path.join(FINAL_DIR, 'video-03-annonceur-complet.mp4');
  const videoOut = path.join(FINAL_DIR, 'video-03-annonceur-complet-voiceover.mp4');
