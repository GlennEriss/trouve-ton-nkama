/**
 * Video 03.3 - Archiver puis reactiver une annonce.
 *
 * Sortie:
 *   videos/video-03-annonceur/final/video-03-03-archiver-reactiver-annonce.mp4
 */
...
function cleanup() {
  for (const file of fs.readdirSync(CAPTURES)) {
    if (file.startsWith('video-03-03-archive')) {
...
async function capture() {
  ...
  await page.locator('text=Studio a louer a Belle Vue 2').scrollIntoViewIfNeeded();
  ...
  const webm = path.join(CAPTURES, 'video-03-03-archive.webm');
  fs.renameSync(tmp, webm);
  return webm;
}

function convert(webm) {
  const out = path.join(FINAL, 'video-03-03-archiver-reactiver-annonce.mp4');
  execSync(
...
async function main() {
  console.log('Capture video-03-03 archiver/reactiver annonce');
