/**
 * Video 09 creation annonce maison IA - Voix off synchronisee.
 *
 * Sorties:
 *   videos/video-09-creation-annonce-maison-ia/voiceover/voiceover-final.mp3
 *   videos/video-09-creation-annonce-maison-ia/final/video-09-creation-annonce-maison-ia-voiceover.mp4
 */
  ...
  return [
    {
      ...
      rate: 0.95,
      ssml: `<speak>
        Vous voulez publier une maison à louer plus rapidement ?
        <break time="0.25s"/>
        Avec l'assistant IA de Trouve Ton Nkama, vous décrivez le logement et vous préparez l'annonce plus facilement.
      </speak>`,
    },
    {
      id: 'capture',
      ...
      rate: 1.03,
      ssml: `<speak>
        Sur la page maison, ouvrez l'assistant IA.
        <break time="0.35s"/>
        Ajoutez les photos du logement, puis décrivez votre maison avec les informations importantes :
        le quartier, le prix, les pièces, le contact et votre rôle sur le bien.
        <break time="0.45s"/>
        Après l'envoi, l'assistant remplit les champs essentiels : le titre, la description, le prix, le statut et les tags.
        <break time="0.35s"/>
        Prenez toujours le temps de vérifier ces informations générées avant de continuer.
        Confirmez aussi le rôle propriétaire direct si c'est votre cas.
        <break time="0.4s"/>
        Complétez les champs manquants, vérifiez la localisation, puis appuyez sur enregistrer.
      </speak>`,
    },
    {
      id: 'outro',
      ...
      ssml: `<speak>
        Trouve Ton Nkama.
        <break time="0.4s"/>
        Publiez vos annonces plus facilement sur tonnkama.com.
      </speak>`,
    },
  ];
}
...
async function main() {
  console.log('Voix off video-09-creation-annonce-maison-ia');
  const padded = [];
  ...
  const videoIn = path.join(FINAL, 'video-09-creation-annonce-maison-ia.mp4');
  const videoOut = path.join(FINAL, 'video-09-creation-annonce-maison-ia-voiceover.mp4');
