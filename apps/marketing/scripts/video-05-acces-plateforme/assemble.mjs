/**
 * Video 05 acces plateforme - Montage final.
 *
 * Sortie:
 *   videos/video-05-acces-plateforme/final/video-05-acces-plateforme-complet.mp4
 */
...
async function makeCard(browser, { id, title, subtitle, duration = 2.4 }) {
        ...
        <style>
          * { box-sizing: border-box; }
          body {
            ...
            background: linear-gradient(180deg, #f7fbfa 0%, #eef8f6 55%, #ffffff 100%);
            color: #103f3d;
            display: flex;
            align-items: center;
...
function concat(parts) {
  const list = path.join(FINAL, 'concat-video-05-acces-plateforme.txt');
  fs.writeFileSync(list, parts.map((file) => `file '${file}'`).join('\n'));
  const out = path.join(FINAL, 'video-05-acces-plateforme-complet.mp4');
  run(
    `ffmpeg -y -f concat -safe 0 -i ${q(list)} -c:v libx264 -preset fast -crf 18 -pix_fmt yuv420p -movflags +faststart ${q(out)}`,
    'Assemblage final video-05-acces-plateforme'
  );
  fs.unlinkSync(list);
  return out;
}
...
async function main() {
  console.log('Montage video-05-acces-plateforme');
  const capture = path.join(FINAL, 'video-05-acces-plateforme-capture.mp4');
  if (!fs.existsSync(capture)) {
    throw new Error(`Fichier manquant: ${capture}`);
  }
  ...
  const parts = [
    await makeCard(browser, {
      ...
      subtitle: 'Voici comment accéder à Trouve Ton Nkama',
      duration: 4,
    }),
    normalizeVideo(capture, '01-capture'),
    await makeCard(browser, {
      id: '02-outro',
      title: 'Trouve Ton Nkama',
      subtitle: 'Disponible sur tonnkama.com',
      duration: 4,
    }),
  ];
  await browser.close();
