...
import { createGeoQuarter } from "@/modules/location-osm/application/gabon-osm-location-management.service";
...
function resolveCreateError(code: string) {
  ...
  if (code.includes("_INVALID") || code.includes("_REQUIRED")) {
    return { status: 400, apiCode: "VALIDATION_ERROR" as const, message: "Données de quartier invalides." };
  }
  return { status: 500, apiCode: "INTERNAL_ERROR" as const, message: "Impossible de créer ce quartier." };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    const quarter = await createGeoQuarter(parsed.data);
    ...
    await logAudit({
      ...
      diff: { operation: "create", quarter },
    });

    return jsonSuccess({ quarter }, auth.correlationId, 201);
  } catch (error) {
