...
import { getAdsExperimentSummary } from "@/modules/analytics-insights/application/ads-analytics-read.service";
import { requireAdmin } from "@/modules/iam/presentation/admin-guard";

const querySchema = z
  .object({
    experimentId: z.string().trim().min(1).max(100),
...
function mapServiceError(error: unknown) {
  ...
  return {
    ...
    message: error instanceof Error ? error.message : "Impossible de charger le résumé d'expérience.",
  };
}
...
export async function GET(request: NextRequest) {
  ...
  const parsed = querySchema.safeParse({
    experimentId: request.nextUrl.searchParams.get("experimentId") ?? undefined,
  ...
  try {
    const result = await getAdsExperimentSummary({
