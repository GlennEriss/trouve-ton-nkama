...
import { buildRecommendationEventsAdapterBatch } from "@/modules/analytics-insights/application/recommendation-analytics-adapter.service";
import { recommendationEventsAdapterBodySchema } from "@/modules/analytics-insights/domain/recommendation-analytics-adapter.schema";
...
export async function POST(request: NextRequest) {
  ...
  const parsedBody = recommendationEventsAdapterBodySchema.safeParse(body);

  if (!parsedBody.success) {
    return jsonError(
      {
        code: "VALIDATION_ERROR",
        message: "Corps de requete adaptateur recommendations/events invalide.",
        details: {
  ...
  const canonicalBatch = buildRecommendationEventsAdapterBatch(parsedBody.data);
  ...
  try {
    const result = await ingestAnalyticsEvents({
      headers: {
        ...
        sourceHeader: auth.sourceHeader,
        ingestedBy: `${auth.ingestedBy}:recommendation-events-adapter`,
      },
      body: canonicalBatch,
      payloadFingerprint,
    });
    ...
    return jsonError(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Echec interne de l'adaptateur recommendations/events.",
      },
      500,
      auth.correlationId,
    );
