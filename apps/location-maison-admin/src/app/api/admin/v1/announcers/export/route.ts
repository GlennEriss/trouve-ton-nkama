...
export async function GET(request: NextRequest) {
    ...
    async start(controller) {
      try {
        ...
        while (exportedRows < maxRows) {
          const page = await listAnnouncers({
            limit: PAGE_SIZE,
            cursor,
          ...
          for (let index = 0; index < page.announcers.length && exportedRows < maxRows; index += 1) {
            const announcer = page.announcers[index];
            const status =
              announcer.state === "ARCHIVED"
                ? "archived"
            ...
            controller.enqueue(
              encoder.encode(
                toCsvLine([
                  ...
                  announcer.roles.join("|"),
                  status,
                  announcer.presenceStatus,
                  announcer.announcerSinceAt ?? "",
                  announcer.lastSeenAt ?? "",
                  announcer.createdAt ?? "",
                ]),
              ),
            );
  ...
  return new Response(stream, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${csvFilename("export-annonceurs")}"`,
